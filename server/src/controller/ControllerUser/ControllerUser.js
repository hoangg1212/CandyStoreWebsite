...
const ModelComments = require('../../model/ModelComment');
const fs = require('fs');
...
class ControllerUser {
    ...
    async SendMessage(req, res) {
        ...
        const dataContact = new ModelContact({
            email: email,
            contactContent: message,
        });
        dataContact.save();
        return res.status(200).json({ message: 'Gủi Liên Hệ Thành Công !!!' });
    }
    ...
    async getContact(req, res) {
        ...
        const decoded = jwtDecode(token);
        if (token) {
            ModelContact.find({ email: decoded.email }).then((data) => res.status(200).json(data));
        }
        return;
    }
